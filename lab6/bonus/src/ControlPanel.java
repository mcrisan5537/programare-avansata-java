...
class ControlPanel extends JPanel {
    ...
    private void undo(ActionEvent actionEvent) {
        DrawingPanel drawingPanel = frame.getDrawingPanel();
        List<DrawingPanel.PairShapeColor> shapes = new ArrayList<>(); // used for drawing shapes
        ...
        if(!drawingPanel.getShapes().isEmpty()) { // if there are more than 0 shapes drawn
            ...
            // paint the lines first, so that the lines are not drawn after the shapes
            for (DrawingPanel.PairShapeColor pair : drawingPanel.getShapes())
                // if the shape is a line, paint it now
                if(pair.shape instanceof Line2D.Double) {
                    drawingPanel.graphics.setColor(pair.color);
                    drawingPanel.graphics.draw(pair.shape);
                // otherwise, paint it later
                } else {
                    shapes.add(pair);
                }

            // now paint the shapes
            for (DrawingPanel.PairShapeColor pair : shapes) {
                drawingPanel.graphics.setColor(pair.color);
